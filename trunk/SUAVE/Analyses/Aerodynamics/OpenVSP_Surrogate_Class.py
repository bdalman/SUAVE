...
class OpenVSP_Surrogate_Class(Aerodynamics):
    """This builds a surrogate and computes lift and drag using OpenVSP
    At the moment it only works if the text file from OpenVSP is already there and documented, so technically it should work for any text file, regardless of source
    Will add on OpenVSP functionality after I figure out the API more
    ...
    def evaluate(self,state,settings,geometry):
        ...
        conditions.aerodynamics.drag_breakdown.untrimmed           = inviscid_drag       
        state.conditions.aerodynamics.inviscid_drag_coefficient    = inviscid_drag

        #print('Lift and drag: ', inviscid_lift, inviscid_drag)
    ...
    def sample_training(self):
        ...
        # Condition input, local, do not keep (k is used to avoid (k)onfusion)
        konditions              = Data()
        ...
        if self.training_file is None:
            ...
            print('Haven\'t built the function to call app yet! Please input text file name as training_file')
            '''
            ...
            print('Got into training file part')
            data_array = np.loadtxt(self.training_file) #Added skiprows so it skips the header (that is saved in the savetxt)
            xy         = data_array[:,0:2]
            CL         = data_array[:,2:3]
            CD         = data_array[:,3:4]
    ...
    def build_surrogate(self):
        ...
        print('Got into build surrogate!')
        #print(xy, CL_data, CD_data)
        ...
        # Resets plot to whatever your actual surrogate range is, plus 20%
