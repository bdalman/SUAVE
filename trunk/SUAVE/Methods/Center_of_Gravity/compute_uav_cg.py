...
# Modified from original: May 2019, B. Dalman
...
def compute_uav_cg(vehicle):
        ...
        vehicle.mass_properties.center_of_gravity = np.zeros(3)
        ...
        if 'vertical_stabilizer' in vehicle.wings:
                ...
                v_tail_mass        = vehicle.mass_properties.tail_weight
                v_tail_cg                 = v_tail.mass_properties.center_of_gravity

                v_tail_moment             = np.linalg.norm(np.add(v_tail.origin, v_tail_cg)) *(v_tail_mass)
        else:
                v_tail_moment = 0
        ...
        propulsor_mass            = vehicle.mass_properties.propulsion_weight
        propulsor_moment          = np.linalg.norm(propulsor_cg)*propulsor_mass
        ...
        if vehicle.fuselages.keys() != []: 
                ...
                fuselage_mass             = vehicle.mass_properties.fuselage_weight
                fuselage_moment           = np.linalg.norm(np.add(fuselage.origin,fuselage_cg))*fuselage_mass 
                '''
                # Furnishings
                ...
                sum_moments              = (wing_moment+h_tail_moment+v_tail_moment+fuselage_moment+\
                                            propulsor_moment)
                ...
                vehicle.mass_properties.center_of_gravity[0]      = (sum_moments)/vehicle.mass_properties.takeoff
                #print(vehicle.mass_properties.center_of_gravity)
