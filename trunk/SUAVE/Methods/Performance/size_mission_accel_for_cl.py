...
def size_mission_accel_for_cl(mission,target_CLs, results=None):
    ...
    -Script currently has issues if the required acceleration is close to 0
    ...
    for i in range(0, num_sections):
        ...
        actual_cd[i] = results.segments[segment_tag].conditions.aerodynamics.drag_coefficient
    ...
    if np.max(np.absolute(resid_cl)) < tol:
        ...
        return results, resid_cl, actual_cl, actual_cd
    ...
    while np.max(np.absolute(resid_cl)) > tol and iteration < maxIter:
        ...
        for i in range(0,num_sections):  
            if old_z_accels[i]==0:
                ...
                delta_z_accels[i] = old_z_accels[i] * resid_cl[i] * (2/np.log(np.absolute(old_z_accels[i]))) #* (first_z_accels[i]**2)/(old_z_accels[i]**2)  #old_z_accel is always (-), so positive resid will push z_accel down, negative resid (actual higher than target) pushes z_accel up

            elif old_z_accels[i]>0:
                #Actual is lower than target
                delta_z_accels[i] = old_z_accels[i] * resid_cl[i] * (-2/np.log(np.absolute(old_z_accels[i]))) #* (first_z_accels[i]**2)/(old_z_accels[i]**2)  #resid is always (+) here, so -0.3 means delta will push towards 
            #else: #for old_z ==0
